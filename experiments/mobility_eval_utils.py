"""
Extended per-episode rollout for the mobility robustness sweep.

Identical field set to experiments/measurement_noise_eval_utils.py
(detection geometry, post-detection metrics, whole-episode evasion, full
operational metrics, estimator metrics including n_position_estimation_samples)
PLUS defender_speed/hostile_speed/mobility_ratio/sweep_axis (mobility sweep
items 13/24). Reuses the SAME physical-limit/numerical-health checks (1e-3
tolerance, unchanged) via experiments.final_eval_utils.
"""
...
def run_mobility_episode(env, policy, seed: int, dt: float, defender_speed: float, hostile_speed: float, sweep_axis: str) -> dict:
    """Run one episode for the mobility sweep and return the full metrics dict."""
