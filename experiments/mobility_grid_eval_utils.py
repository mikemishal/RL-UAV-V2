"""
Extended per-episode rollout for the defender-hostile speed interaction grid.

Superset of experiments/mobility_eval_utils.py's field set (detection
geometry, post-detection metrics, whole-episode evasion, full defender
operational metrics, estimator metrics with n_position_estimation_samples)
PLUS, per the mobility-dynamics audit's findings (items 27/28):
  - hostile dynamics diagnostics (speed, saturation fractions, path length,
    boundary-hit counts)
  - lightweight 3-D geometry diagnostics (horizontal vs vertical enemy-soldier
    separation, since the audit showed these can decouple at high v_e).
...
def run_mobility_grid_episode(env, policy, seed: int, dt: float, defender_speed: float, hostile_speed: float) -> dict:
    """Run one episode for the defender-hostile speed interaction grid and
    return the full metrics dict (dt is the environment's fixed timestep)."""
    ...
    # 3-D geometry diagnostics (item 28)
    min_horizontal_enemy_soldier_dist = float("inf")
    altitude_at_min_horizontal_dist = None
    min_full_3d_enemy_soldier_dist = float("inf")
    entered_2m_horizontal_while_vertical_gt_2m = False

    def _update_geometry(enemy_pos, soldier_pos):
        nonlocal min_horizontal_enemy_soldier_dist, altitude_at_min_horizontal_dist
        nonlocal min_full_3d_enemy_soldier_dist, entered_2m_horizontal_while_vertical_gt_2m
        ...
        if horiz < min_horizontal_enemy_soldier_dist:
            min_horizontal_enemy_soldier_dist = horiz
            altitude_at_min_horizontal_dist = float(enemy_pos[2])
        if dist3d < min_full_3d_enemy_soldier_dist:
            min_full_3d_enemy_soldier_dist = dist3d
        if horiz <= 2.0 and abs(dz) > 2.0:
            entered_2m_horizontal_while_vertical_gt_2m = True

    _update_geometry(info["enemy_pos"], info["soldier_pos"])
    ...
    row = {
        ...
        "mobility_ratio": defender_speed / hostile_speed,
        ...
        "min_horizontal_enemy_soldier_dist": float(min_horizontal_enemy_soldier_dist),
        "altitude_at_min_horizontal_enemy_soldier_dist": (
            float(altitude_at_min_horizontal_dist) if altitude_at_min_horizontal_dist is not None else float("nan")
        ),
        "min_full_3d_enemy_soldier_dist": float(min_full_3d_enemy_soldier_dist),
        "entered_2m_horizontal_while_vertical_gt_2m": 1 if entered_2m_horizontal_while_vertical_gt_2m else 0,
