"""
Parameter sweep: Measurement noise vs success probability (Kalman baseline).

Evaluates KalmanGreedyInterceptPolicy with use_kalman_tracking=True across
shared measurement-noise values. Noise values represent measurement variance
(sigma^2), not standard deviation.

Usage:
    python experiments/baseline/sweep_noise_kalman_baseline.py \
        [--n-episodes N] [--noise-values "0.0,0.1,0.25,0.5,1.0,2.0"]

Outputs:
    - CSV:  results/baseline/sweep_noise_kalman_baseline.csv
    - Plot: results/baseline/sweep_noise_kalman_baseline.png
"""
...
def sweep_noise_kalman_baseline(
    noise_values: list[float],
    n_episodes: int,
    seed_offset: int = 0,
    verbose: bool = True,
) -> pd.DataFrame:
    """Run a Monte Carlo noise sweep for the Kalman greedy baseline."""
    if verbose:
        print("=" * 76)
        print("PARAMETER SWEEP: MEASUREMENT NOISE (Kalman Baseline)")
        print("=" * 76)
    ...
    for noise_value in noise_values:
        ...
        def env_factory(var=noise_value):
            config = EnvConfig(use_kalman_tracking=True, measurement_var=var)
...
def plot_sweep(df: pd.DataFrame, output_path: str, show: bool = False) -> None:
    ...
    ax.errorbar(
        ...
        color="#F39C12",
        ecolor="#4A4A4A",
        label="Kalman Baseline (95% CI)",
    )

    ax.set_xlabel("Measurement Noise Variance (sigma^2)", fontsize=12)
    ax.set_ylabel("Success Probability", fontsize=12)
    ax.set_title("Kalman Baseline: Success Probability vs Measurement Noise", fontsize=13)
    ax.set_ylim(0.0, 1.0)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Parameter sweep: measurement noise vs success probability (Kalman baseline)"
    )
    parser.add_argument(
        "--n-episodes",
        type=int,
    ...
    df = sweep_noise_kalman_baseline(
    ...
    csv_path = os.path.join(output_dir, "sweep_noise_kalman_baseline.csv")
    plot_path = os.path.join(output_dir, "sweep_noise_kalman_baseline.png")

    df.to_csv(csv_path, index=False)
    print(f"\nResults saved to: {csv_path}")

    print("\n" + "=" * 110)
    print("NOISE SWEEP RESULTS (Kalman Baseline)")
    print("=" * 110)
    print(
