"""
Extended per-episode rollout for the detection-radius robustness sweep.

Combines: acquisition/detection geometry + post-detection interception
metrics (as in experiments/acquisition_eval_utils.py) with WHOLE-EPISODE
evasion-activation metrics and full operational metrics (as in
experiments/final_eval_utils.py) -- see
experiments/robustness_detection_protocol.py items 17-21 for the exact
required field list. Reuses the SAME physical-limit/numerical-health checks
(1e-3 tolerance, unchanged) via experiments.final_eval_utils.
"""
...
def run_robustness_episode(env, policy, seed: int, dt: float) -> dict:
    """Run one episode for the detection-radius sweep and return the full metrics dict."""
    ...
    # Pre-detection-scoped evasion (item 18)
    evasion_active_before_detection = False
    first_evasion_step = None
    # Whole-episode-scoped evasion (item 20)
    ...
    row = {
        ...
        # Operational metrics (item 20) -- whole-episode evasion, per this task's spec.
        ...
        # Estimator metrics (item 21) -- identical definition across both tracks.
