...
\(\theta_{\max}=30^\circ\).
The results depend only on \(u_t^{\mathrm{Lead}}\) being a unit nominal
direction and therefore also apply when the Lead controller uses its normalized
direct-pursuit fallback because no valid analytical intercept solution exists.
